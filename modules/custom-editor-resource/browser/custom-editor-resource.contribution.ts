...
export class CustomEditorResourceContribution
  implements BrowserEditorContribution, CommandContribution
{
  ...
  registerEditorComponent(registry: EditorComponentRegistry) {
    ...
    registry.registerEditorComponentResolver(
      'custom-resource',
      (
        _resource: IResource<any>,
        _results: IEditorOpenType[],
  ...
  registerEditorDocumentModelContentProvider(
    ...
    registry.registerEditorDocumentModelContentProvider(
      this.customSchemeDocumentProvider,
    );
  }
      ...
      },
      {
        execute: () => {
          this.editorService.open(
            new URI('custom-resource://path/to/resource?resourceId=1'),
            { preview: false },
          );
        },
      },
