...
import styles from './builtin-services.module.less';
...
export const BuiltinServicesView = () => {
  ...
  const showDialog = React.useCallback(async () => {
    const res = await dialogService.open('Hello', MessageType.Info, [
      'Cancel',
      'OK',
    ]);
  ...
  return (
    <div className={styles.components_wrap}>
      ...
      <Button
        ...
        onClick={() => showMessage('hello', MessageType.Info)}
      >
        Show Info Message
      </Button>
      <Button
        style={{ marginRight: 10 }}
        onClick={() => showMessage('hello', MessageType.Error)}
      >
        Show Error Message
      </Button>
      <Button onClick={() => showMessage('hello', MessageType.Warning)}>
