...
export class CustomEditorMonacoContribution
  implements BrowserEditorContribution
{
  ...
  registerEditorComponent(registry: EditorComponentRegistry) {
    ...
    registry.registerEditorComponentResolver(
      ...
      (resource, results) => {
        if (
          resource.uri.path.ext === `.${'jsonconfig'}` ||
          customFileRegex.test(resource.uri.path.toString())
        ) {
          results.push({
            type: 'component',
            componentId: COMPONENTS_ID,
  ...
  registerEditorDocumentModelContentProvider(
    ...
    registry.registerEditorDocumentModelContentProvider(
      this.customEditorDocumentContentProvider,
    );
  }
