...
export const TokenCustomEditorDocumentContentProvider = Symbol(
...
@Injectable()
export class CustomEditorDocumentContentProvider
  implements IEditorDocumentModelContentProvider
{
  ...
  async provideEditorDocumentModelContent(
    ...
    const { content: buffer } = await this.fileServiceClient.readFile(
      fileURI.toString(),
    );
    ...
    const jsonData = fakeDBServer.getJsonData(
      dbName,
      userName,
      userToken,
      defaultTable,
