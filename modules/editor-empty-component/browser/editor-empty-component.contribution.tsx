...
export const EditorEmptyComponent = () => {
  ...
  const keybindingRegistry =
    useInjectable<KeybindingRegistry>(KeybindingRegistry);
  ...
  const getKeybinding = (commandId: string) => {
    ...
    const keyBindings = bindings.sort(
  ...
  return (
    <div className={styles.empty_component}>
      <img src={LOGO_URL} onLoad={() => setImgLoaded(true)} />
      {ShortcutView}
    </div>
  );
};
