...
import { LIBRO_COMPONENTS_SCHEME_ID } from '../../browser/libro.protocol';
...
export class Keybindhandler implements ApplicationContribution {
  ...
  registerContextKey() {
    const contextKeyService: IContextKeyService =
      this.injector.get(IContextKeyService);
    const workbenchEditorService: WorkbenchEditorService = this.injector.get(
      WorkbenchEditorService,
    );
    const notebookFocusContext = contextKeyService.createKey<boolean>(
  ...
  registerCommand() {
    const sumiCommandRegistry: SumiCommandRegistry =
      this.injector.get(SumiCommandRegistry);
  ...
  registerKeybind() {
    const keybindingService: KeybindingRegistry =
      this.injector.get(KeybindingRegistry);
